...
def plot_stock_data(df):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['Close'].rolling(window=20).mean(), line=dict(color='orange', width=1), name="20-day MA"), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['Close'].rolling(window=50).mean(), line=dict(color='green', width=1), name="50-day MA"), row=1, col=1)
    colors = ['red' if float(df['Open'].iloc[idx]) > float(df['Close'].iloc[idx]) else 'green' for idx in range(len(df))]
    fig.add_trace(go.Bar(x=df.index, y=df['Volume'], marker_color=colors, name="Volume"), row=2, col=1)
    fig.update_layout(title=f'{df.name} Stock Price', xaxis_rangeslider_visible=False, height=600, template="plotly_white")
    return fig
